 ...
 * =============
 *
 * Execudes the calculation for the julia-fractal. This will
...
void main()
{
	...
	float nX = dimension.x / 2.0;
	float nY = dimension.y / 2.0;

	// x- and y-coordinate of the specific pixel in the canvas
	float x = ((4.0/zoom.x)/dimension.x)*(nX-gl_FragCoord.x)*(-1.0);
	float y = ((3.0/zoom.y)/dimension.y)*(nY-gl_FragCoord.y);
	...
	for (int i = 1; i <= MAX_STEPS; i++)
	{
		...
		// See the definition of the julia set to understand this condition
